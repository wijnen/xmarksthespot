...
contents. Engines MAY include more members inside the Wherigo module
for internal use. Anything that is not defined in this document MUST NOT be
used by Authors or Builders. This includes creating new objects. Engines
MUST NOT use any variable outside of the Wherigo module directly.
...
\funcdef{ZObject.made (object)}{boolean}{object: the object to be tested}{Tests whether an object is made by (is an instance of) this class. It will only return true if the tested object is of the calling class, or of a subclass of it. This is a class-method, it does not take a self object as first argument, and must not be called with `:'.}
...
\attrdef{Container}{ZObject}{ZObject that contains this object. Always a Zone, or ZItem. This attribute is read-only; it is updated automatically when using MoveTo.}
...
\attrdef{Inventory}{Table of ZObject}{A list of all objects which have this ZObject set as their Container. This attribute is read-only; it is updated automatically when using MoveTo.}
...
\attrdef{On\textit{CommandName}}{Function}{These event callbacks are available for ZItem and Zone objects, once for each ZCommand that is available for it. \textit{CommandName} is the name of the command. It is triggered by selecting the command. See ZCommand for details.}
...
\funcdef{ZCartridge ()}{ZCartridge}{-}{Constructor. Unlike other ZObject subclasses, this constructor does not need to receive the Game Object (for obvious reasons).}
\funcdef{zcartridge:RequestSync ()}{nil}{}{Save the current state of the Game. More information on saving is below.}
\funcdef{zcartridge:GetAllOfType (type)}{Table of ZObjects}{String: type name to search for}{Return all items in zcartridge.AllZObjects of the requested type.}
\attrdef{AllZObjects}{Table of ZObject}{List of all ZObjects in this Cartridge. This attribut is read-only. It is automatically updated when ZObjects are created or destroyed.}
...
\attrdef{OnRestore}{Function}{Event, called immediately after loading a saved game.}
...
\attrdef{Commands}{Table of ZCommand}{Commands which may be available with this ZItem.}
...
\attrdef{OriginalPoint}{ZonePoint}{The point which is supposed to be the origin of the Zone; it MUST NOT be on the perimiter. It is by definition inside the Zone.}
...
\attrdef{State}{String}{State of the Player with respect to this Zone. One of
	\textit{Inside}, \textit{Proximity}, \textit{Distant}, \textit{NotInRange}. The Player is inside the zone when Player.ObjectLocation is within the perimiter. Otherwise, if the distance to the perimiter is smaller than ProximityRange, the State is Proximity. If it is larger than ProximityRange, but smaller than DistanceRange, the State is Distant. Otherwise it is NotInRange. If not specified in the constructor, the initial value of State is NotInRange. If this is not the actual state at that time, callbacks are immediately called as if the Player has moved from NotInRange to their current location.}
...
\attrdef{Remaining}{Number}{Time in seconds before it expires. If the timer is running, this value will automatically count down.}
...
Helper class for commands on ZItems and Zones. When these are displayed, it may
...
\attrdef{WorksWithAll}{Boolean}{Only used if CmdWith is true. If this is true, every Visible ZItem, except the object that this command is for, will automatically be in the list of available targets.}
...
\funcdef{Dialog (table)}{nil}{table: a list of messages.}{Show a list of messages to the player, in sequence. The elements of the table are the same as for the MessageBox function, except that they cannot have a Callback or Buttons (the Engine would normally give them an implicit Ok button to show the next message).}
...
contains images they must be in a sudirectory named Image.
...
an Image, or Sound field (depending on the file location) is defined as the
